...
struct MenuSlider: SwiftUI.View {
	...
	private var maxValue: Double {
		return Double(self.items.count)
	}
	...
	private func changeAngle(location: CGPoint) {
		...
		let newProgress = ((positiveAngle / (2.0 * .pi)) * (maxValue - self.minValue)) + self.minValue
	...
	private func scale(for index: Int) -> CGFloat {
		...
		let distance = min(abs(progress - Double(index)), itemCount - abs(self.progress - Double(index)))
